...
export default function HomeScreen() {
  ...
  // A lista agora é populada com os dados validados do Firestore
  const [petsDisponiveis, setPetsDisponiveis] = useState<any[]>([]);
  const [carregando, setCarregando] = useState(false);

  // BUSCA DADOS NO FIREBASE COM FILTRO DE SEGURANÇA
  const buscarPetsHome = async () => {
    setCarregando(true);
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        // Só adiciona se o pet tiver nome, evitando documentos corrompidos/vazios
        if (dados && dados.nome) {
          listaPets.push({ id: doc.id, ...dados });
        }
      });
      setPetsDisponiveis(listaPets);
    } catch (error) {
      console.error("Erro ao buscar pets na home:", error);
    } finally {
      setCarregando(false);
    }
  };

  // Atualiza a lista toda vez que a tela Home ganha foco
  useFocusEffect(
    useCallback(() => {
      buscarPetsHome();
    }, [])
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.sectionContainer}>
        ...
        {carregando ? (
          <ActivityIndicator size="large" color="#8DC4A6" />
          ...
          >
            {petsDisponiveis.map((pet: any) => (
              <TouchableOpacity key={pet.id} style={styles.cardPet} onPress={() => router.push('/adotar')}>
                <Image source={{ uri: pet.fotos ? pet.fotos[0] : pet.foto }} style={styles.petImage} />
                <View style={styles.petInfo}>
                  <Text style={styles.petCardName}>{pet.nome}</Text>
                  <Text style={styles.petCardAge}>{pet.idade} anos</Text>
                </View>
              </TouchableOpacity>
            ))}
          </ScrollView>
